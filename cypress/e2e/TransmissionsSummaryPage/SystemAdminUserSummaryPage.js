...
import TransmissionsSummaryObjects from "../pages/TransmissionsSummaryObjects.js";
...
describe("System Admin User Transmission Page", function () {
    ...
    it("Verify Transmission Summary page details", function () {
        ...
        transmissionPage.getTableData(true, 'State').invoke('text').then((state) => {
            transmissionPage.getTableData(true, 'Report Period').invoke('text').then((reportPeriod) => {
                transmissionPage.getTableData(true, 'File Type').invoke('text').then((fileType) => {
                    transmissionPage.elements.firstTableLink().invoke('text').then((fileNum) => {
                        ...
                        transmissionsSummaryObjects.elements.transmissionDetailsHeader().should('have.text', 'Transmission Details');
                        commonPage.elements.headerH3Text().should('have.text', `File #${fileNum}`);
                        transmissionsSummaryObjects.elements.transmissionHeader().should('have.text', `${reportPeriod} ${state} - ${fileType}`);
                    });
                });
            });
        });
    ...
    it("Verify the fields and links in the Total Penalty Calculations card with a non-0% penalty", function () {
        ...
        commonPage.clickOnRefreshResultBtn();
        transmissionPage.getTableData(true, 'Penalty').invoke('text').then((potentialPenalty) => {
            transmissionPage.elements.firstTableLink().click();
            transmissionsSummaryObjects.elements.penaltyCardHeader().should('have.text', 'Total Penalty Calculations');
    ...
    it("Verify system admin user can submit transmissions", function () {
        cy.visit('/User.html');
        commonPage.clickOnTransmissionsTab();
        transmissionPage.elements.firstTableLink().click();
        ...
        commonPage.elements.headerH3Text().invoke('text').then((fileNum) => {
            ...
            transmissionPage.elements.successModalBtn(cleanedFileNum).should('have.text', 'Return to Transmissions Page').click();
        });
    
    });
    it("Verify system admin user can delete transmissions", function () {
        cy.visit('/User.html');
        commonPage.clickOnTransmissionsTab();
        transmissionPage.elements.firstTableLink().click();
