...
describe("State Authorized Official Account Request Validations", function () {
  ...
  it("Deny Request as SAO", function () {
    ...
    cy.get(':nth-child(3) > [data-testid="header_dropdown_button"]').should("have.text", 'Account Settings').click();
    cy.get(':nth-child(2) > .styles_button__dXuFt').click();
    cy.get('[data-testid="manage_user_account_requests"]').click();
    cy.get('td').contains('FName').should('exist');
    cy.get('td').contains('FName').should('exist');
  ...
  it("Deny Request as SA", function () {
    cy.standardAdminLogin('nytdsysadmin', 'P@ssw0rd');
    cy.get(':nth-child(3) > [data-testid="header_dropdown_button"]').should("have.text", 'Account Settings').click();
    cy.get(':nth-child(2) > .styles_button__dXuFt').click();
    cy.get('[data-testid="manage_user_account_requests"]').click();
    cy.get('td').contains('FName').should('exist');
